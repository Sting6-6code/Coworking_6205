    ...
    private void loadUserData() {
        try (BufferedReader br = new BufferedReader(new FileReader(DATA_FILE))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] f = line.split(",");
                // CSV格式：userId,username,password,email,type,membership
                if (f.length >= 6) {
                    String userId = f[0];
                    String username = f[1];
                    // 匹配username（因为currentUser是username）
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (f.length >= 6) {
                    ...
                    if (username.equals(currentUser)) {
                        ...
                        // 重新构建行：userId,newUsername,updatedPwd,newEmail,type,currentMembership
